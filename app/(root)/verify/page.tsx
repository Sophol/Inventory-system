...
export default async function QRVerifyProductPage({ searchParams }: QRVerifyProductPageProps) {
  const awaitedSearchParams = await searchParams;
  const code = awaitedSearchParams.p ?? "";
        ...
        {code ? (
          isVerified ? (
            <div className="inline-flex flex-col items-center bg-green-900 rounded-xl px-6 py-6 shadow-lg text-green-200 select-none">
              <span className="text-4xl mb-4 animate-pulse drop-shadow-lg">✅</span>
              ...
              <h2 className="text-lg font-bold text-green-100">Sample Product Name</h2>
              <h2 className="text-lg font-bold text-green-100">Product Serial</h2>
            </div>
          ) : (
